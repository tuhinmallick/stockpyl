...
#		network = load_instance_old("example_6_1", "/Users/larry/Documents/GitHub/stockpyl/src/stockpyl/datasets/stockpyl_instances_old.json")
		...
		txt_filename = f'{filename_root}.txt'
		csv_filename = f'{filename_root}.csv'
...
class TestWriteInstanceAndStates(unittest.TestCase):
	...
	def test_example_6_1(self):
		...
		self.assertListEqual(
			new_network.get_node_from_index(0).demand_source.demand_list[:T],
			demand_list[:T],
		)
	...
	def test_example_6_1_with_disruptions(self):
		...
		self.assertListEqual(
			new_network.get_node_from_index(
				1
			).disruption_process.disruption_state_list[:T],
			disruption_list[:T],
		)
	...
	def test_rong_atan_snyder_figure_1a(self):
		...
		demand_list = {
			node.index: [node.state_vars[t].inbound_order[None] for t in range(T)]
			for node in network.nodes
		...
		for node in new_network.nodes:
			if node in new_network.sink_nodes:
				self.assertListEqual(
					node.demand_source.demand_list[:T], demand_list[node.index][:T]
				)
		# Remove state variables and demand sources and make sure networks are equal otherwise.
		for node in network.nodes:
			node.demand_source = None
	...
	def test_rong_atan_snyder_figure_1a_with_disruptions(self):
		...
		for node in new_network.nodes:
			...
			if node.disruption_process is not None and node.disruption_process.random_process_type is not None:
				self.assertListEqual(
					node.disruption_process.disruption_state_list[:T],
					disruption_state_list[node.index][:T],
				)
		# Remove state variables, demand sources, and disruption processes and make sure networks are equal otherwise.
		for node in network.nodes:
			node.demand_source = None
